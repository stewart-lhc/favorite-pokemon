...
- 声明提交使用隐藏 honeypot 和 Neon 双层固定窗口限频（单客户端与网络总量）；限频表只保存 HMAC 指纹、窗口与计数，不保存原始 IP 或 User-Agent。
...
本地 API 需要配置 `DATABASE_URL`，指向 Neon Postgres。schema 在 `db/schema.sql`。已有数据库在部署本次反滥用改动前，需执行：

```powershell
npm run db:migrate
```

生产环境可额外配置独立的 `RATE_LIMIT_SALT`；未配置时会使用现有 `DATABASE_URL` 作为 HMAC 密钥来源，两者都不会写入限频表或日志。
